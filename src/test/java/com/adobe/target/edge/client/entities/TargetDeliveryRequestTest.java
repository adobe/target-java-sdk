...
class TargetDeliveryRequestTest {
  ...
  private void verifyId(
      Map<String, CustomerState> customerIds, TargetDeliveryRequest targetDeliveryRequest) {
  ...
  private void verifyVisitorState(
      TargetDeliveryResponse targetDeliveryResponse,
  ...
  private void verifyAnalyticsValues(TargetDeliveryRequest targetDeliveryRequest) {
    AnalyticsRequest analytics =
        targetDeliveryRequest.getDeliveryRequest().getExperienceCloud().getAnalytics();
  ...
  private void validateCustomerId(
      Map<String, CustomerState> customerIdMap, List<CustomerId> customerIds) {
    ...
    customerIds.forEach(
        customerId -> {
          ...
          assertEquals(
              customerState.getAuthState().ordinal(), customerId.getAuthenticatedState().ordinal());
        });
  }

  private void verifyServerStateAndNewCookie(
      TargetDeliveryResponse targetDeliveryResponse, String newSessionId) {
