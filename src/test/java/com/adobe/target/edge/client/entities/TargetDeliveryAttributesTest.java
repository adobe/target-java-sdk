...
class TargetDeliveryAttributesTest {
  ...
  void init() throws IOException, NoSuchFieldException {
    ...
    RuleLoader testRuleLoader =
        TargetTestDeliveryRequestUtils.getTestRuleLoaderFromFile(
            "DECISIONING_PAYLOAD_ATTRIBUTES.json");
  ...
  @SuppressWarnings("unchecked")
  private void validateInitialResponse(
      TargetDeliveryRequest targetDeliveryRequest, Attributes attrs) {
    assertNotNull(attrs);
    assertNotNull(attrs.getResponse());
    assertEquals(200, attrs.getResponse().getStatus());
