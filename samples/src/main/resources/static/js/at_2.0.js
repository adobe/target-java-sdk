...
function shouldUseOptin() {
  ...
  return isOptinEnabled(index, optinEnabled);
}
function isTargetApproved() {
  return isCategoryApproved(index, TARGET);
}
function isAnalyticsApproved() {
  return isCategoryApproved(index, ANALYTICS);
}
function fetchOptinPermissions() {
  return fetchPermissions(index, TARGET);
}
...
var $ = (function (window) {
  var Zepto = function () {
    ...
    function executeScript(doc, content) {
          ...
          nodes.forEach(function (node) {
            ...
            if (parentInDocument) traverseNode(node, function (el) {
              if (el.nodeName != null && el.nodeName.toUpperCase() === "SCRIPT" && (!el.type || SCRIPT_TYPES.test(el.type.toLowerCase())) && !el.src) {
                executeScript(document, el.innerHTML);
              }
            });
          });
        });
...
function initNamespace() {
  ...
  index[NAMESPACE].querySelectorAll = select;
}
function handleAuthoringTriggeredView(options) {
  ...
  index[NAMESPACE][CURRENT_VIEW] = viewName;
}
function setupClickHandler() {
  index$1.addEventListener(CLICK, function (event) {
    if (isFunction(index[NAMESPACE][EDITOR])) {
      index[NAMESPACE][EDITOR](event);
    }
  }, true);
}
...
function createContext(context) {
  ...
  return {
    userAgent: index.navigator.userAgent,
...
function createDeliveryRequest(request, visitorValues, allParams) {
  var deviceId = getDeviceId();
...
function createAsyncDeliveryRequest(request, params) {
  return getAsyncThirdPartyData().then(function (arr) {
    ...
    return createDeliveryRequest(request, visitorValues, allParams);
  });
}
...
function executeRequestInternal(request, requestTimeout) {
  var config = getConfig();
  var url = createRequestUrl(config);
  var headers = defineProperty({}, CONTENT_TYPE, [TEXT_PLAIN]);
  var timeout$$1 = getTimeout(config, requestTimeout);
  var async = true;
  var options = { url: url, headers: headers, body: request, timeout: timeout$$1, async: async };
...
function executeRequest$1(request, requestTimeout) {
  if (!shouldUseOptin() || isTargetApproved()) {
    return executeRequestInternal(request, requestTimeout);
  }
...
function processPageLoad(httpContext) {
  ...
  return all([processOptions$1(options, isValidOption), processMetrics(metrics, isValidSelectorMetric)]).then(createOptionsMetricsPair).then(function (result) {
    if (isNil(response)) {
      return null;
    }
    if (!isNil(analytics)) {
...
function getAnalyticsFromItem(item) {
...
function getAnalyticsDetails(response) {
  ...
  var views = prefetch.views || [];
  var pageLoadDetails = getAnalyticsFromItem(pageLoad);
  var mboxesDetails = flatten(map(getAnalyticsFromItem, mboxes));
  var prefetchMboxesDetails = flatten(map(getAnalyticsFromItem, prefetchMboxes));
  var viewsDetails = flatten(map(getAnalyticsFromItem, views));
...
function processResponse(httpContext) {
  ...
  var promises = [pageLoad, mboxes, prefetchMboxes, views, metrics];
...
function awaitUsingMutationObserver(selector, timeout$$1, queryFunc) {
  return create(function (res, rej) {
    var mo = new MO_OBJECT(function () {
    ...
    mo.observe(index$1, MO_CONFIG);
  });
}
function canUseRequestAnimation() {
  return index$1[VISIBILITY_STATE] === VISIBLE;
}
function awaitUsingRequestAnimation(selector, timeout$$1, queryFunc) {
  return create(function (res, rej) {
    function execute() {
      var elems = queryFunc(selector);
      if (!isEmpty(elems)) {
      ...
      index.requestAnimationFrame(execute);
    }
    execute();
    delay(function () {
      rej(createError$1(selector));
...
function awaitUsingTimer(selector, timeout$$1, queryFunc) {
  return create(function (res, rej) {
    function execute() {
      var elems = queryFunc(selector);
      if (!isEmpty(elems)) {
...
function awaitSelector(selector) {
  var timeout$$1 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : getConfig()[SELECTORS_POLLING_TIMEOUT];
  ...
  if (canUseMutationObserver()) {
    return awaitUsingMutationObserver(selector, timeout$$1, queryFunc);
  }
  if (canUseRequestAnimation()) {
    return awaitUsingRequestAnimation(selector, timeout$$1, queryFunc);
  }
  return awaitUsingTimer(selector, timeout$$1, queryFunc);
}
...
function decorateActions(actions, key, page) {
  return map(function (e) {
    return index$3({ key: key, page: page }, e);
  }, actions);
}
...
function hideAllOptions(context) {
...
function executeApplyOffer(options) {
  ...
  hideAllOptions(renderingContext);
...
function executeApplyOffers(options, skipPrehiding) {
...
function handleSuccess(response, skipPrehiding) {
  logDebug(INIT, RESPONSE, response);
  addClientTrace({ source: INIT, response: response });
  executeApplyOffers({ selector: HEAD_TAG, response: response }, skipPrehiding)['catch'](handleError$1);
}
function processServerState(config) {
  ...
  addClientTrace({ source: INIT, serverState: serverState });
  hideAllOptions(response);
  processResponse({ request: request, response: response }).then(function (res) {
...
function sendNotifications(options) {
  ...
  if (!validationResult[VALID]) {
    ...
    addClientTrace({ source: SEND_NOTIFICATIONS, options: options, error: error });
    return;
  }
  if (!isDeliveryEnabled()) {
  ...
  addClientTrace({ source: SEND_NOTIFICATIONS, options: options });
  var notifications = request.notifications;
  var notificationsRequest = createNotificationRequest(consumerId, {}, notifications);
...
window.adobe.target.init(window, document, {
  "clientCode":"emeaprod4",
  "imsOrgId":"0DD934B85278256B0A490D44@AdobeOrg",
  "serverDomain":"emeaprod4.tt.omtrdc.net",
  "timeout": 2000,
